...
from customize_240501_testCMSHLT3156 import customizeHLTforThroughputMeasurement
...
for ntry in {00..01}; do
  ...
  ./start-mps-daemon.sh
  sleep 1
  run "${jobLabel}"_dump.py ./patatrack-scripts "${jobDirPrefix}"-gpu_mps-vanilla-j08t32s24 8 32 24
  ./stop-mps-daemon.sh
  sleep 1
  ...
  ./start-mps-daemon.sh
  sleep 1
  run "${jobLabel}"_dump.py ./patatrack-scripts "${jobDirPrefix}"-gpu_mps-vanilla-j04t32s24 4 32 24
  ./stop-mps-daemon.sh
  sleep 1
  ...
  ./start-mps-daemon.sh
  sleep 1
  run "${jobLabel}"_dump.py ./patatrack-scripts "${jobDirPrefix}"-gpu_mps-vanilla-j08t16s12 8 16 12
  ./stop-mps-daemon.sh
  sleep 1
