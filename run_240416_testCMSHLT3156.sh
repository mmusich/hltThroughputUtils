...
hltConfigFromDB --configName /online/collisions/2024/2e34/v1.1/HLT/V4 > "${jobLabel}"_cfg.py

cp /gpu_data/store/data/Run2024C/EphemeralHLTPhysics/FED/pileup60/run"${runNumber}"_cff.py .

cat <<@EOF >> "${jobLabel}"_cfg.py

process.load('run${runNumber}_cff')

from customize_240416_testCMSHLT3156 import customizeHLTforThroughputMeasurement
